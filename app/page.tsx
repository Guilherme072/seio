...
const LoadingSkeleton = () => (
    <div className="flex items-center justify-center py-24"><div className="relative"><div className="w-12 h-12 rounded-full border-2 border-transparent border-t-[#8A4BFF] animate-spin"></div><div className="absolute inset-0 w-12 h-12 rounded-full border-2 border-transparent border-r-[#DB9EFF] animate-spin animation-delay-150"></div></div></div>
);
        ...
        <div className="max-w-7xl mx-auto p-10 space-y-12">
          <div className="flex items-center justify-between">
            <div className="space-y-3">
              ...
              <p className="eclipse-text-secondary text-xl">Gerencie contatos de marcas, agências, influenciadores e freelancers</p>
            </div>
            <Button className="eclipse-button-primary h-14 px-8 rounded-xl font-semibold flex items-center gap-3 text-base" disabled>
    ...
    <div className="min-h-screen eclipse-bg eclipse-scroll">
      <div className="max-w-7xl mx-auto p-10 space-y-12">
        <div className="flex items-center justify-between animate-in fade-in slide-in-from-top-4">
          ...
          <Button onClick={() => setShowAddDialog(true)} className="eclipse-button-primary h-14 px-8 rounded-xl font-semibold flex items-center gap-3 text-base">
            <Plus className="h-6 w-6" /> Novo Contato
          </Button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-6">
          {[
            ...
            <Card key={stat.title} className="eclipse-stats-card eclipse-card-hover rounded-xl animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${index * 100}ms` }}>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-12">
          <TabsList className="grid w-full grid-cols-7 p-2 eclipse-card rounded-2xl h-16">
            {[ { value: "todos", label: "Todos" }, { value: "marcas", label: "Marcas" }, { value: "bets", label: "Bets" }, { value: "agencias", label: "Agências" }, { value: "influenciadores", label: "Influenciadores" }, { value: "pessoas-influentes", label: "Pessoas Influentes" }, { value: "freelancers", label: "Freelancers" }, ].map((tab) => ( <TabsTrigger key={tab.value} value={tab.value} className="eclipse-tab rounded-xl h-12 font-medium transition-all duration-300 text-sm">{tab.label}</TabsTrigger> ))}
          </TabsList>
        ...
        <AddBrandDialog open={showAddDialog} onOpenChange={setShowAddDialog} onAdd={(newBrand) => { setBrands([...brands, { ...newBrand, id: brands.length + 1 }]); setShowAddDialog(false); }} />
        <BrandDetailsModal brand={selectedBrand} open={!!selectedBrand} onClose={() => setSelectedBrand(null)} onUpdate={(updatedBrand) => { setBrands(brands.map((b) => (b.id === updatedBrand.id ? updatedBrand : b))); setSelectedBrand(updatedBrand); }} />
      </div>
    </div>
  );
}
