...
export function BrandDetailsModal({
  ...
  const [editingContactData, setEditingContactData] = useState<
    Partial<Contact>
  >({});
  const [addingContactMethod, setAddingContactMethod] = useState<number | null>(
    null
  );
  ...
  const safeObservations = Array.isArray(brand.observations)
    ? brand.observations
    : [];
  const safeContacts = Array.isArray(brand.contacts) ? brand.contacts : [];
  const safeSuggestedInfluencers = Array.isArray(brand.suggestedInfluencers)
  ...
  const addObservation = () => {
    if (!brand || !newObservation.trim()) return;
    onUpdate({
      ...brand,
      observations: [
        ...safeObservations,
  ...
  const saveContactEdit = () => {
    ...
    onUpdate({
      ...brand,
      contacts: safeContacts.map((c) =>
        c.id === editingContact ? { ...c, ...editingContactData } : c
      ),
    });
  ...
  const deleteContact = (contactId: number) => {
    ...
    onUpdate({
      ...brand,
      contacts: safeContacts.filter((c) => c.id !== contactId),
    });
  };
  const addContactMethod = (contactId: number) => {
    if (!newContactMethod.type || !newContactMethod.value || !brand) return;
    onUpdate({
      ...brand,
  ...
  const handleAddContact = (
    newContact: Omit<Contact, "id" | "addedBy" | "addedDate">
  ) => {
    if (!brand) return;
    ...
    onUpdate({ ...brand, contacts: [...safeContacts, fullContact] });
  };
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-[1024px] max-h-[90vh] overflow-y-auto p-0 border-0 eclipse-scroll">
            ...
            <DialogHeader className="p-8 pb-6 text-left">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <DialogTitle className="eclipse-title text-2xl font-bold">
                      {brand.name}
                    </DialogTitle>
                    <DialogDescription asChild>
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-[#FBF9FF] text-sm">
                          {brand.category}
                        </span>
                        <Badge
                          className={`text-xs px-3 py-1 rounded-full eclipse-glow-soft ${getStatusColor(
                            brand.status
                          )}`}
                        >
                          {brand.status}
                ...
                <Button
                  ...
                  size="icon"
                  onClick={onClose}
                  className="eclipse-button rounded-xl absolute top-6 right-6 z-20"
                >
                  <X className="h-5 w-5" />
                </Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="eclipse-card eclipse-card-hover rounded-2xl p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={`text-xs px-3 py-1 rounded-full eclipse-glow-soft ${getRelationshipColor(
                ...
                <div className="eclipse-card eclipse-card-hover rounded-2xl p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="eclipse-text-secondary font-bold">
                        {closedDeals}
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  {brand.lastContact && (
                    <div className="text-xs text-[#FBF9FF] opacity-70">
                      Último contato:{" "}
                      {new Date(brand.lastContact).toLocaleDateString("pt-BR")}
                      <br />
                      Por{" "}
                      <strong className="eclipse-text-secondary">
                        {brand.lastContactBy}
                      </strong>{" "}
                      com{" "}
                      <strong className="eclipse-text-secondary">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="eclipse-title text-xl font-bold">
                    Contatos da Marca ({safeContacts.length})
                  </h3>
                </div>
                <Button
                  size="sm"
              ...
              {safeContacts.length > 0 ? (
                <div className="space-y-6">
                  {safeContacts.map((contact) => (
                    <div
                      key={contact.id}
                      className="eclipse-card eclipse-card-hover rounded-2xl p-6"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          {editingContact === contact.id ? (
                            <div className="space-y-4">
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  <Label className="text-[#FBF9FF] text-sm">
                                    Nome
                                  </Label>
                                  <Input
                            ...
                            <>
                              <div className="flex items-start justify-between mb-3">
                                <div>
                                  ...
                                  <p className="text-[#FBF9FF] text-sm">
                                    {contact.role}
                                  </p>
                                ...
                                {Array.isArray(contact.contactMethods) &&
                                  contact.contactMethods.map((method, idx) => (
                                    ...
                                    >
                                      ...
                                      <span className="text-[#FBF9FF] capitalize">
                                        {method.type}:
                                      </span>
                                      <span className="eclipse-text-secondary">
                                        {method.value}
                                      </span>
                                    </div>
                                  ))}
                              ...
                              {addingContactMethod === contact.id ? (
                                ...
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    setAddingContactMethod(contact.id)
                                  }
                                  className="eclipse-button rounded-xl"
                                >
                                  <Plus className="h-4 w-4 mr-2" />
                              ...
                              <div className="text-xs text-[#FBF9FF] opacity-50 mt-3">
                                Adicionado por {contact.addedBy} em{" "}
                ...
                <div className="eclipse-card eclipse-card-hover rounded-2xl p-12 text-center">
                  ...
                  <h3 className="eclipse-title text-lg font-bold mb-2">
                    Nenhum contato cadastrado
                  </h3>
                  <p className="text-[#FBF9FF] mb-6">
                    Adicione o primeiro contato desta marca.
                  </p>
                  <Button
                    onClick={() => setShowAddContact(true)}
                    className="eclipse-button-primary rounded-xl px-8"
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Adicionar Contato
                  </Button>
            ...
            <div className="p-8">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="eclipse-title text-xl font-bold">
                    Observações ({safeObservations.length})
                  </h3>
                </div>
                <Button
                  size="sm"
              ...
              <div className="space-y-4">
                {safeObservations.map((obs) => (
                  <div
                    ...
                    className="eclipse-card eclipse-card-hover rounded-2xl p-6"
                  >
                    <p className="text-[#FBF9FF] leading-relaxed mb-3">
                      {obs.text}
                    </p>
              ...
              {safeObservations.length === 0 && !showObservationForm && (
                <div className="eclipse-card eclipse-card-hover rounded-2xl p-12 text-center">
                  ...
                  <p className="text-[#FBF9FF]">
                    Adicione observações importantes sobre esta marca.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="space-y-3">
                    {previousInfluencers.map((influencer, index) => (
                      <div
                        key={index}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <Badge className="eclipse-badge-success text-xs px-3 py-1 rounded-full">
                            Fechado
                          </Badge>
                        </div>
                      </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="eclipse-title text-xl font-bold">
                      ...
                      {safeSuggestedInfluencers.length})
                    </h3>
                  </div>
                  <div className="space-y-3">
                    {safeSuggestedInfluencers.map((influencer, index) => (
                      <div
                        key={index}
                        className="eclipse-card eclipse-card-hover rounded-xl p-4 cursor-pointer transition-all duration-300"
                        onClick={() => setSelectedInfluencer(influencer)}
                  ...
                  {safeSuggestedInfluencers.length === 0 && (
                    <div className="eclipse-card eclipse-card-hover rounded-2xl p-8 text-center">
                      ...
                      <p className="text-[#FBF9FF] text-sm">
                        Nenhum influenciador sugerido para esta marca.
                      </p>
                    </div>
                  )}
                </div>
